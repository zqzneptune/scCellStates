...
class RegularizedNNLSProjector(StateProjector):
    ...
    def __init__(self, vocabulary, *, l1: float = 0.0, l2: float = 0.0):
        ...
        if min(l1, l2) < 0 or not np.isfinite(l1 + l2):
            raise ProjectionError("l1 and l2 must be finite and nonnegative")
        if l1 == 0 and l2 == 0:
            raise ProjectionError("at least one regularization penalty must be positive")
    ...
    def transform(self, adata, *, layer=None, sample_id="projection"):
        ...
        for row in range(adata.n_obs):
            ...
            def objective(h):
                return (
                    0.5 * np.sum((values - basis @ h) ** 2)
                    + self.l1 * np.sum(h)
                    + 0.5 * self.l2 * np.sum(h * h)
                )
            ...
            fit = minimize(
                objective,
                np.zeros(self.vocabulary.n_programs),
                jac=gradient,
                bounds=[(0, None)] * self.vocabulary.n_programs,
                method="L-BFGS-B",
            )
            if not fit.success:
                raise ProjectionError(f"regularized projection failed for cell {row}: {fit.message}")
            usages[row] = fit.x
        return make_result(
            adata,
            matrix,
