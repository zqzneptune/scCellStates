...
class SimplexProjector(StateProjector):
    ...
    def transform(self, adata, *, layer=None, sample_id="projection"):
        ...
        constraints = {"type": "eq", "fun": lambda h: np.sum(h) - 1.0,
                       "jac": lambda h: np.ones(k)}
        for row in range(adata.n_obs):
            ...
            fit = minimize(objective, initial, jac=gradient, bounds=[(0, 1)] * k,
                           constraints=constraints, method="SLSQP",
                           options={"ftol": 1e-12, "maxiter": 500})
            if not fit.success:
                raise RuntimeError(f"simplex projection failed for cell {row}: {fit.message}")
            usages[row] = fit.x
